...
"""Cetus-style: search WASM memory for known score value, find towers nearby."""
...
screen = js(2, "(document.body.innerText||'').match(/GuestInPeace[\\s\\t]*(\\d+)/)")
print("Screen:", screen, flush=True)


score_search = js(3, """
(function(){
  var score = parseInt((document.body.innerText||'').match(/GuestInPeace[\\\\s\\\\t]*(\\\\d+)/)[1]);
  var dv = new DataView(window.__kbExp.memory.buffer);
  var candidates = [];
  for(var off = 0; off < dv.byteLength - 4; off += 4){
    var v = dv.getUint32(off, true);
    if(v === score){
      candidates.push(off);
      if(candidates.length >= 50) break;
    }
  }
  return JSON.stringify({score: score, count: candidates.length, candidates: candidates.slice(0, 20)});
})()""")
print("Score search:", score_search, flush=True)